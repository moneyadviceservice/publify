module Admin::FeedbackHelper
  ...
  def button_to_edit_comment(item)
    link_to(content_tag(:span, '', class: 'glyphicon glyphicon-pencil'), edit_admin_feedback_path(item), class: 'btn btn-primary btn-xs btn-action')
  end

  def button_to_delete_comment(item)
    link_to(content_tag(:span, '', class: 'glyphicon glyphicon-trash'), remove_admin_feedback_path(item), class: 'btn btn-danger btn-xs btn-action')
  end
  ...
  def change_status(item, context = 'listing')
    ...
    link_to(content_tag(:span, '', class: "glyphicon glyphicon-thumbs-#{klass}"), change_state_admin_feedback_path(item, context: context), class: "btn btn-#{klass2} btn-xs btn-action", remote: true)
  end
