  ...
  describe 'scopes' do
    describe '#without_images' do
      it 'should list resource that are not image (based on mime type)' do
        other_resource = FactoryBot.create(:resource, mime: 'text/css')
        image_resource = FactoryBot.create(:resource, mime: 'image/jpeg')
    ...
    describe '#by_filename' do
      it 'should sort resource by filename' do
        b_resource = FactoryBot.create(:resource, upload: file_upload('b'))
        a_resource = FactoryBot.create(:resource, upload: file_upload('a'))
    ...
    describe '#by_created_at' do
      it 'should sort resource by created_at DESC' do
        b_resource = FactoryBot.create(:resource, created_at: DateTime.new(2011, 03, 13))
        a_resource = FactoryBot.create(:resource, created_at: DateTime.new(2011, 02, 21))
    ...
    describe '#without_images_by_filename' do
      it 'should combine 2 scopes' do
        image_resource = FactoryBot.create(:resource, mime: 'image/jpeg')
        b_resource = FactoryBot.create(:resource, mime: 'text/html', upload: file_upload('b'))
        a_resource = FactoryBot.create(:resource, mime: 'text/html', upload: file_upload('a'))
